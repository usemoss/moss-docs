...
Moss retrieving is so fast that it injects results into the LLM context before the user finishes their turn. This eliminates the need for slow "tool calling" loops, ensuring natural and seamless interactions.
  ...
  <Step title="Build the Pipeline">
    ...
    async def run_bot(transport: BaseTransport, runner_args: RunnerArguments):
        ...
        pipeline = Pipeline([
            ...
            #--------------Moss Integration----------------
            moss_service.query(os.getenv("MOSS_INDEX_NAME"), top_k=int(os.getenv("MOSS_TOP_K", "5"))),
            #---------------------------------------------
            llm,
            tts,
            transport.output(),
...
| `alpha` | `float` | `0.8` | **Hybrid Search Weighting**. <br/>`0.0` = Keyword only (BM25). <br/>`1.0` = Semantic only (Vector). <br/>`0.8` is recommended for most voice use cases. |