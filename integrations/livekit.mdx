...
Searches happen locally on-device. Your sensitive data never needs to leave the agent.
...
You can now connect to this agent using the LiveKit Playground or your own frontend.
...
The `alpha` parameter controls the balance between Keyword Search (BM25) and Semantic Search (Vector). You can adjust this dynamically based on the query length. Short queries often benefit from keyword matching, while long questions benefit from semantic understanding.
