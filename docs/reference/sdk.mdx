...
- `QueryOptions` – `{ top_k?: int; alpha?: float; filters?: dict }`
...
# Build query presets (use within your existing async workflow)
keyword_query = QueryOptions(top_k=3, alpha=0.0)
default_query = QueryOptions(top_k=3)  # alpha defaults to semantic-heavy (e.g., ~0.8)
semantic_query = QueryOptions(top_k=3, alpha=1.0)

# Example usage inside your async code:
await client.query("faqs", "return policy", keyword_query)
await client.query("faqs", "return policy", default_query)
await client.query("faqs", "return policy", semantic_query)
```
</CodeGroup>
